...
function PageRouter({ page, navigate, navigationState, updateState, refreshUpdateState, appVersion, settings }) {
  const props = { onNavigate: navigate, navigationState, updateState, refreshUpdateState, appVersion, settings };
  switch (page) {
        ...
        <div className="app-shell">
          <aside className="sidebar" data-testid="sidebar">
            ...
            <nav className="sidebar-nav">
              {NAV_ITEMS.map(item => (
                <button key={item.key} className={`nav-btn ${page === item.key ? 'active' : ''}`} onClick={() => navigate(item.key, null)} data-testid={`nav-${item.key}`} title={`Open ${item.label}`}>
